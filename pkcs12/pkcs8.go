...
func ParsePKCS8PrivateKey(der []byte) (key interface{}, err error) {
	...
	case privKey.Algo.Algorithm.Equal(oidPublicKeyECDSA):
		...
		// If the curve is sm2, we need to convert the key to sm2.PrivateKey
		if key.(*ecdsa.PrivateKey).Curve == sm2.P256Sm2() {
			sm2Key := new(sm2.PrivateKey)
			sm2Key.PublicKey.Curve = sm2.P256Sm2()
			sm2Key.D = key.(*ecdsa.PrivateKey).D
			sm2Key.PublicKey.X = key.(*ecdsa.PrivateKey).X
			sm2Key.PublicKey.Y = key.(*ecdsa.PrivateKey).Y
			return sm2Key, nil
		}
...
func parseECPrivateKey(namedCurveOID *asn1.ObjectIdentifier, der []byte) (key *ecdsa.PrivateKey, err error) {
	...
	k := new(big.Int).SetBytes(privKey.PrivateKey.Bytes)
	...
	for len(privKey.PrivateKey.Bytes) > len(privateKey) {
		if privKey.PrivateKey.Bytes[0] != 0 {
			return nil, errors.New("x509: invalid private key length")
		}
		privKey.PrivateKey.Bytes = privKey.PrivateKey.Bytes[1:]
	}

	// Some private keys remove all leading zeros, this is also invalid
	// according to [SEC1] but since OpenSSL used to do this, we ignore
	// this too.
	copy(privateKey[len(privateKey)-len(privKey.PrivateKey.Bytes):], privKey.PrivateKey.Bytes)
...
func MarshalPrivateKey(key *sm2.PrivateKey, oid asn1.ObjectIdentifier) ([]byte, error) {
	...
	PrivateKeyRawValue := asn1.RawValue{
		Class:      asn1.ClassUniversal,
		Tag:        asn1.TagInteger,
		IsCompound: false,
		Bytes:      paddedPrivateKey,
	}
	return asn1.Marshal(ecPrivateKey{
		Version:       1,
		PrivateKey:    PrivateKeyRawValue,
		NamedCurveOID: oid,
